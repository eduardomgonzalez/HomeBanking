...
var nombreUsuario = "Eduardo González";
var saldoCuenta = 5000;
var limiteExtraccion = 800;
...
function cambiarLimiteDeExtraccion() {
    ...
    if(isNaN(nuevoLimite)) //Validacion si el valor no es numerico
    {
        alert("Debe ingresar un monto numérico.");

    }else if(nuevoLimite <= 0){ //Validacion si el valor ingresado es menor o igual a 0
        alert("Debe ingresar un monto mayor a 0.");

    }else if(nuevoLimite <= saldoCuenta){ //Validacion para que no me permita fijar un limite superior a mi saldo
        limiteExtraccion = nuevoLimite;
        actualizarLimiteEnPantalla();
        alert("El nuevo límite de extracción es: $" + limiteExtraccion);

        }else{
            alert("No se puede fijar este límite de extracción porque es superior al saldo de su cuenta.");
        }    
}
...
function extraerDinero() {
    ...
    if(isNaN(montoAExtraer)) //Validacion si el valor no es un numero
    {
        alert("Debe ingresar un monto numérico.");

    }else if(montoAExtraer <= 0){ //Validacion si el monto es menor o igual a 0
        alert("Debe ingresar un monto mayor a 0");
        
    }else{
        if(haySaldoDisponible(montoAExtraer) == true && hayLimiteDeExtraccion(montoAExtraer) == true
...
function depositarDinero() {
    ...
    var montoADepositar = parseInt(prompt("Ingrese el dinero que desea depositar:"));
    
    if(isNaN(montoADepositar)) //Validacion si no es un numero
    {
        alert("Debe ingresar un monto numérico.");
    
    }else if(montoADepositar <= 0){ //Validacion si el monto es menor o igual a 0
        alert("Debe ingresar un monto mayor a $0.");
        ...
        alert("Saldo anterior: $"+ montoActual + "\n" +
              "Dinero depositado: $" + montoADepositar +"\n" +
              "Saldo actual: $"+saldoCuenta);
    }
}
...
function transferirDinero() {
    ...
    if(isNaN(montoATransferir))
    {
    ...
    }else if(montoATransferir <= 0){ //Validacion monto menor o igual a 0
        alert("El monto a transferir debe superior a $0.");

    }else if(montoATransferir > saldoCuenta){ //Validacion si el monto es superior al saldo
        alert("No tiene el saldo suficiente para transferir esa cantidad de dinero.");

    }else{
        var numCuenta = parseInt(prompt("Ingrese el numero de cuenta a la que desea transferir el dinero:"));
        ...
        if(numCuenta != cuentaAmiga1 && numCuenta != cuentaAmiga2)
        {
            ...
            actualizarSaldoEnPantalla();
            alert("Se ha trasnferido: $" + montoATransferir + ".\n" +
...
function iniciarSesion() {
    ...
    }else{ //Funcionalidad extra de opcion de 3 intentos para ingresar el password

        var intentos = 3;

        while(passwordIngresado != 1234 && intentos <= 3 && intentos >=1)
        {
            passwordIngresado = parseInt(prompt("ATENCION! Tiene " + intentos + " intentos mas para ingresar su password, " +
